...
)
{
	...
	case FS_FAT12 :
		...
		ofs = bc % 512; bc /= 512;
		if (ofs != 511) {
			if (fs->ops.disk_readp(buf, fs->fatbase + bc, ofs, 2)) break;
		} else {
			if (fs->ops.disk_readp(buf, fs->fatbase + bc, 511, 1)) break;
			if (fs->ops.disk_readp(buf+1, fs->fatbase + bc + 1, 0, 1)) break;
		}
		wc = LD_WORD(buf);
		return (clst & 1) ? (wc >> 4) : (wc & 0xFFF);
#endif
	case FS_FAT16 :
		if (fs->ops.disk_readp(buf, fs->fatbase + clst / 256, (WORD)(((WORD)clst % 256) * 2), 2)) break;
		return LD_WORD(buf);
#if _FS_FAT32
	case FS_FAT32 :
		if (fs->ops.disk_readp(buf, fs->fatbase + clst / 128, (WORD)(((WORD)clst % 128) * 4), 4)) break;
...
)
{
	...
	if (res != FR_OK)
		...
		return FR_NOT_READY;
	else if(!(FatFs->ops.disk_readp))
		return FR_DISK_ERR;

	do {
		res = FatFs->ops.disk_readp(dir, dj->sect, (WORD)((dj->index % 16) * 32), 32)	/* Read an entry */
			? FR_DISK_ERR : FR_OK;
		if (res != FR_OK) break;
...
{
	if (fs->ops.disk_readp(buf, sect, 510, 2))		/* Read the boot sector */
		return 3;
	if (LD_WORD(buf) != 0xAA55)				/* Check record signature */
		return 2;

	if (!fs->ops.disk_readp(buf, sect, BS_FilSysType, 2) && LD_WORD(buf) == 0x4146)	/* Check FAT12/16 */
		return 0;
	if (_FS_FAT32 && !fs->ops.disk_readp(buf, sect, BS_FilSysType32, 2) && LD_WORD(buf) == 0x4146)	/* Check FAT32 */
		return 0;
	return 1;
}
...
)
{
	...
	else if(!(fs->ops.disk_initialize))
		return FR_DISK_ERR;
	else if(!(fs->ops.disk_readp))
		return FR_DISK_ERR;

	if (fs->ops.disk_initialize() & STA_NOINIT)	/* Check if the drive is ready or not */
		return FR_NOT_READY;
	...
	if (fs->ops.disk_readp(buf, bsect, 13, sizeof(buf)))
	       	return FR_DISK_ERR;
...
)
{
	...
	*br = 0;
	...
	while (btr)	{			/* Repeat until all data transferred */
		...
		dr = fs->ops.disk_readp(!buff ? 0 : rbuff, fs->dsect, (WORD)(fs->fptr % 512), rcnt);
		if (dr) goto fr_abort;
...
)
{
	...
	*bw = 0;
	...
	if (!btw) {		/* Finalize request */
		if ((fs->flag & FA__WIP) && fs->ops.disk_writep(0, 0)) goto fw_abort;
		fs->flag &= ~FA__WIP;
		return FR_OK;
	...
	while (btw)	{			/* Repeat until all data transferred */
		if (((WORD)fs->fptr % 512) == 0) {		/* On the sector boundary? */
			...
			fs->dsect = sect + cs;
			if (fs->ops.disk_writep(0, fs->dsect)) goto fw_abort;	/* Initiate a sector write operation */
			fs->flag |= FA__WIP;
		}
		wcnt = 512 - ((WORD)fs->fptr % 512);		/* Number of bytes to write to the sector */
		if (wcnt > btw) wcnt = btw;
		if (fs->ops.disk_writep(p, wcnt)) goto fw_abort;	/* Send data to the sector */
		fs->fptr += wcnt; p += wcnt;			/* Update pointers and counters */
		btw -= wcnt; *bw += wcnt;
		if (((WORD)fs->fptr % 512) == 0) {
			if (fs->ops.disk_writep(0, 0)) goto fw_abort;	/* Finalize the currtent secter write operation */
			fs->flag &= ~FA__WIP;
		}
	}
