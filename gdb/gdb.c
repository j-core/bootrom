...
void gdb_putmsg (char c, const char *buf, int len)
	{
	unsigned char sum;
...
#if 0
/* Notifications do not seem to work. I'm not sure whether I've
   formatted them incorrectly or if GDB is in the wrong mode. Take
   care when trying to use this in the future. */
void gdb_putnotification (const char *name, int name_len, const char *data, int data_len)
	{
	unsigned char sum;

	gdb_putc('%');
	sum = putbuf(name_len, name);
	sum += gdb_putc(':');
	sum += putbuf(data_len, data);
...
/*
  ...
  "Txx..........."
*/
void gdb_expedited_response (int sigval)
	{
	long val;
	int id = 0;
	int reglen;
...
void expedited_response (int sigval)
	{
	do
		{
		gdb_expedited_response(sigval);
		}
	while ('+' != gdb_getc());
