...
int gdb_startup (void) {
  /* Before calling main, see if GDB is connected. Wait for a message
     or a break character. */
  unsigned char c;
  enum gdb_msg_state state = OUTSIDE_MSG;

  unsigned int start_seconds = rtc_get_seconds_lo();
  /* Read incoming data either to see a break byte or a GDB pkt.
     Timeout after 6 seconds */

  while (abs((int)(rtc_get_seconds_lo() - start_seconds)) < 6) {
    if (uart_rx_no_block(UART_DEV, &c) == 0) {
      switch (c) {
      case 3:
        /* 3 is the break signal sent by GDB when you press Ctrl-C
           once */
        goto enter_gdb;
      case '$':
        /* $ begins the GDB remote messages */
        state = IN_DATA;
        break;

      case '#':
        if (state == IN_DATA) {
          state = BEFORE_CHKSUM_1;
        }
        break;

      default:
        if (state == BEFORE_CHKSUM_1) {
          /* could validate the checksum characters are hex and valid,
             but plan to send NAK for packet anyway */
          state = BEFORE_CHKSUM_2;
        } else if (state == BEFORE_CHKSUM_2) {
          /* packet is complete. Send NAK and jump to gdb stub. */
          gdb_putc('-');
          goto enter_gdb;
        }
        break;
  ...
  return 0;
 enter_gdb:
  /* gdb is entered through a trapa in entry */
  return 1;
}
